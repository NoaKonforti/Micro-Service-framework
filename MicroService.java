...
public abstract class MicroService implements Runnable {
    ...
    private static MessageBus instanceMessageBus = MessageBusImpl.getInstance();
    private int id = -1;
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        instanceMessageBus.subscribeEvent(type,this);
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        instanceMessageBus.subscribeBroadcast(type,this);
        callbacks.put(type,callback);
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        return instanceMessageBus.sendEvent(e);
    }
    ...
    protected final void sendBroadcast(Broadcast b) {
        instanceMessageBus.sendBroadcast(b);
    }
    ...
    protected final <T> void complete(Event<T> e, T result) {
        instanceMessageBus.complete(e, result);
    }
    ...
    public final void run() {
        instanceMessageBus.register(this);
        initialize();
        while (!terminated) {
            try {
                Message message = instanceMessageBus.awaitMessage(this);
                callbacks.get(message.getClass()).call(message);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }
        instanceMessageBus.unregister(this);
    }
}
